...
public class MilchItem extends ItemFood
{
    ...
    public MilchItem(int var1, int var2, int var3, float var4, boolean var5)
    {
    ...
    public ItemStack onEaten(ItemStack var1, World var2, EntityPlayer var3)
    {
        ...
        var3.sendChatToPlayer("Your drunklevel is:" + DrunkEffect.getDrunkLevel());

        if (DrunkEffect.getDrunkLevel() > 0)
        {
            DrunkEffect.DrunkLevelMinus();
            Side var4 = FMLCommonHandler.instance().getEffectiveSide();

            if (var4 == Side.SERVER)
            {
                EntityPlayerMP var5 = (EntityPlayerMP)var3;
                PacketDispatcher.sendPacketToPlayer(PacketHandler.getPortPacket(4, 0, 0, 0), (Player)var5);
            }
            else if (var4 == Side.CLIENT)
            {
                EntityClientPlayerMP var6 = (EntityClientPlayerMP)var3;
                PacketDispatcher.sendPacketToServer(PacketHandler.getPortPacket(4, 0, 0, 0));
            }
            else
            {
